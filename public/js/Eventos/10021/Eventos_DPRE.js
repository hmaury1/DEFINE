    ...
    validateField:function (e){
      ...
      if(e.field.name=="HOJA"){
        if(e.value=="0"){
           ...
           if(nhojas>0){ 
              Ext.Msg.alert("Verifique","Verifique","Prestamo o es parcial o total pero no ambos!. Verifique");
              e.value="";
           }
        } else {
          TotalHojas = SM.Cursor("C03DPRE"," IDPLANO='"+e.record.get("IDPLANO")+"'");
          if(e.record.get("HOJA") > TotalHojas){ 
              Ext.Msg.alert("Verifique","Verifique","HOJA no existe. Verifique TotalHojas en Plano");
              e.value="";
          } else { 
           nhojas = SM.Cursor("C02DPRE"," IDESTADODETPRESTAMO=26 And IDPLANO="+ e.record.get("IDPLANO")+" And HOJA='"+e.record.get("HOJA")+"' ");
            if(nhojas > 0){  
              Ext.Msg.alert("Verifique","Verifique","HOJA ya fue prestada o todo el plano. Verifique");
              e.value="";
            }
          } 
        } 
